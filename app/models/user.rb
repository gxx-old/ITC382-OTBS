...
class User < ActiveRecord::Base
  ...
  validates_uniqueness_of   :login, :email, :case_sensitive => false, :allow_blank => true
  validates_format_of :email, :with => /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\Z/i, :message => "must be a valid email address.",
                      :allow_blank => true
  validates_format_of :login, :with => /\A[a-z0-9_-]+\Z/i, :message => "must contain alphanumerical characters or -_ only.", :allow_blank => true
